...
title_size=19;  %TITLE SIZE SETTINGS
 ...
 %%*********************** plot pressione del cuscinetto, raggio di curvatura e profondita` di penetrazione           
        ...
        %% radius
%         figure;
%         plot(time(1:end-1),r_n,'r--','Linewidth',1);
%         hold on
...
%         
        ...
        figure;
        plot(time(1:end-1),d_n,'r','Linewidth',2);
        hold on;grid on;
        plot(time(1:end-1),delta_R_n,'k--','Linewidth',2);
        %plot(time(1:end-1),state_sim(1:end-1,1),'g--','Linewidth',2);
        plot(time(1:end-1),position_n,'b--','Linewidth',2);
...
%         title(['Contact contributions: Penetration depth , reference type: ',rif_type,', Simulation: ',sim_type],'FontSize',title_size,'Interpreter', 'none')
        ...
        figure;
        plot(time(1:end-1),pc_n_f./10^5,'k','Linewidth',2);
        hold on;grid on;
        plot(time(1:end-1),input_u_n./10^5,'r','Linewidth',2);
...
%         
        ...
        plot(time(1:end-1),state_sim(1:end-1,1),'b','Linewidth',1);
        hold on; grid on;
        plot(time(1:end-1),input_u_n./10^5,'r','Linewidth',1);
